<template>
  <div class="container p-5">
    <div class="tag has-background-mediumBlue has-text-white mb-5 is-medium">New Dissolution Proposal</div>
    <!-- PAPER PROPOSAL FORM -->
    ...
    <div class="field" v-if="!preview">
      ...
      <div class="control">
        <input class="input" v-model="token" type="text" placeholder="Enter token address here">
      </div>
    </div>
    <div class="field" v-if="!preview">
        <label class="label">Purchase Amount</label>
        <div class="control">
          <input class="input" v-model="purchaseAmount" type="number" placeholder="Enter token address here">
        </div>
      </div>
    <div class="field" v-if="!preview">
    ...
    <div class="field" v-if="!preview">
      ...
      <div class="control">
        <input type="checkbox" class="checkbox" v-model="daoResolution" /><span
          class="ml-1 has-text-mediumGray is-italic"> - check this if the proposal will make changes to the DAO</span>
      </div>
    </div>
    <div v-if="preview">
    ...
    <div class="block">
      <div :class="[preview ? 'is-primary ' : 'is-secondary ', 'button has-text-white is-size-5 p-3']"
        @click=togglePreview>
        <span class="mr-2">
          <unicon height="18" width="18" fill="white" :name="preview ? 'pen' : 'eye'"></unicon>
    ...
    <div class="block is-flex is-justify-content-space-between mt-5">
      ...
      <button @click="publish" class="button has-background-success has-text-white has-text-weight-bold">Submit
        Proposal</button>
    </div>
    <!-- End Form -->
  </div>
...
export default {
  ...
  methods: {
    ...
    async publish() {
      ...
      await this.createDissolutionProposal({ pType: "Dissolution", assetAddr, proposalType, title, description, daoResolution, token, purchaseAmount, forumLink, $toast: this.$toast });
      this.$emit("proposed");
    },
