<template>
  <div class="container p-5">
    <div class="tag has-background-mediumBlue has-text-white mb-5 is-medium">New Governor Change Proposal</div>
    <!-- PAPER PROPOSAL FORM -->
    <div v-if="!preview">
      <div class="field">
        <label class="label">New Governor</label>
        <div class="control">
...
export default {
  ...
  name: "newGovernorChangeProposal",
  components: {
    Proposal
  },
  emits: ["submitted", "proposed"],
  data() {
    return {
      address: "",
      title: "",
      ...
      proposal: null,
      preview: false,
      proposalType: ProposalTypes.GovernorChange
    }
  },
  computed: {
    assetId() {
  ...
  methods: {
    ...mapActions({
      createGovernorChangeProposal: "createProposal",
    }),
    togglePreview() {
    ...
    async publish() {
      ...
      this.$emit("submitted");
      const props = {
        pType: "GovernorChange",
        assetId: this.assetId,
        governor: this.address,
        title: `Change the governor to ${this.address}`,
        description: this.description,
        forumLink: this.forumLink.includes("https://forum.weavr.org/") ? this.forumLink : "https://forum.weavr.org/c/dao-proposals/",
        $toast: this.$toast
      }
      await this.createGovernorChangeProposal(props);
      this.$emit("proposed");
    },
    onCancel() {
      this.$router.back();
