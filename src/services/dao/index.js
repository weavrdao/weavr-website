...
class DAO {
   ...
   * @param assetId
   * @param {string} governor Address of the participant
   * @param {string} title Title of the proposal (might be autogenerated)
   * @param {string} description Proposal body
   * @param {string} forumLink Link to forum discussion
   * @returns {Boolean} Transaction status (true — mined; false - reverted)
   */
  async createGovernorChangeProposal(assetId, title, governor, description, forumLink) {
    ...
    console.log("INFOHASH___", infoHash)
    console.log(assetId, governor);
    if (!infoHash) return;

    let status = await assetContract.proposeGovernorChange(governor, infoHash);
    return status;
  }
